...
- [ ] T001 Create `frontend/src/lib/shortcuts.ts` — shortcut definition type (id, keys,
...
- [ ] T002 Create `frontend/src/lib/useKeyboardShortcuts.ts` — React hook that mounts the
  ...
  pending sequence).
- [ ] T003 Create `frontend/src/lib/kbd.ts` — pure display helpers: `kbdKey(key)` and
  `modLabel()` returning `⌘` on macOS and `Ctrl` otherwise (spec §7 FR-08, §12; the
  "Platform" testing requirement §17.7).
- [ ] T004 Create `frontend/src/lib/modalLayer.ts` — modal-presence registry:
...
- [ ] T005 Unit tests: `frontend/tests/component/shortcuts.test.ts` (`isTypingContext`
...
- [ ] T006 Modify `frontend/src/components/Dialog.tsx` — in the existing `useEffect([open])`,
  ...
  behavior unchanged).
- [ ] T007 Component test (`modal-layer.test.ts` or `dialog.test.tsx`): mounting an open
...
- [ ] T008 Modify `frontend/src/components/Layout.tsx` — remove the inline `/`+`Ctrl/Cmd+K`
  ...
  help dialog.
- [ ] T009 Add the quiet Help affordance in the sidebar footer (button labelled "Keyboard
  ...
  where interactive.
- [ ] T010 Component tests: new `frontend/tests/component/layout-shortcuts.test.tsx` — `/`
...
- [ ] T011 Modify `frontend/src/pages/IssuePage.tsx` — register contextual bindings via the
  ...
  composer).
- [ ] T012 Component tests: extend `frontend/tests/component/issue-page.test.tsx` — `E`
...
- [ ] T013 Create `frontend/src/components/KeyboardShortcutsDialog.tsx` — composes the
...
- [ ] T014 [P] Add the `.kbd` rule to `frontend/src/styles/components.css` — mono
...
- [ ] T015 Component tests: `frontend/tests/component/keyboard-shortcuts-dialog.test.tsx` —
...
- [ ] T016 Axe: extend `frontend/tests/accessibility/core.test.tsx` — the open
...
- [ ] T017 Full verification: `npm run typecheck -w frontend`, `npm run lint -w frontend`,
...
- [ ] T018 Walk the Visual Review Checklist (§19, §10); update `frontend/VISUAL_LANGUAGE.md`
...
- [ ] T019 Regression re-run of the **complete** frontend suite (incl. `layout-search.test.tsx`,
