...
- [x] T001 In `frontend/src/pages/WorkspacePage.tsx`, split the row so `Link` wraps only
...
- [x] T002 In `frontend/src/pages/MyIssuesPage.tsx`, apply the same split (additionally
  preserving `ledger-select` checkbox sibling and `ledger-context` caption).
- [x] T003 [P] Adjust `frontend/src/styles/components.css` so the visual run is identical:
...
- [x] T004 Verify `frontend/tests/component/workspace-page.test.tsx` and the My Issues
...
- [x] T005 Create `frontend/src/components/quickEdit.ts` — shared types + state helper
  (`QuickEditField`, `QuickEditState = { issueId, field } | null`) enforcing one-edit-at-a-
  time semantics (D-06).
- [x] T006 Create `frontend/src/components/QuickEditSelect.tsx` — badge-styled trigger
...
- [x] T007 Create `frontend/src/components/QuickEditPopover.tsx` — anchored hairline surface
...
- [x] T008 [P] Style both in `frontend/src/styles/components.css` using existing tokens only
...
- [x] T009 Component tests for primitives (new test file under `frontend/tests/component/`):
...
- [x] T010 Wire `quickEdit` state into `frontend/src/pages/WorkspacePage.tsx` and replace
  the status badge with `QuickEditSelect` (`ISSUE_STATUSES`; commit body `{ status }`).
- [x] T011 Replace the priority badge with `QuickEditSelect` (`ISSUE_PRIORITIES`; body
  `{ priority }`).
- [x] T012 Replace the assignee chip with `QuickEditSelect` over already-fetched `members`
...
- [x] T013 Replace the label badges with a labels control (first badge / `+N more` entry
  point) opening `QuickEditPopover` over already-fetched workspace `labels`; confirm PATCHes
  the full `{ labelIds }` set (D-07).
- [x] T014 Add a compact due-date chip in `ledger-meta` (real date or quiet `Due`
  placeholder — never fabricated, §13.1) opening the date variant of `QuickEditPopover`;
  commit `{ dueDate: 'YYYY-MM-DD' | null }`.
- [x] T015 Implement the commit path: disable control while in flight (D-12) →
...
- [x] T016 Verify selection checkboxes, bulk actions, filters, and active saved view are
  unaffected by a Quick Edit interaction; no navigation occurs (§13.5).
- [x] T017 Component tests (`workspace-page.test.tsx` extensions): per-field change updates
...
- [x] T018 Verify `MyIssuesPage` `members`/`wsLabels` coverage per row's `workspaceId`;
...
- [x] T019 Apply T010–T016 patterns to `frontend/src/pages/MyIssuesPage.tsx` verbatim,
  preserving `ledger-context` and mixed-workspace bulk-disable behavior.
- [x] T020 Component tests for the My Issues ledger mirroring T017.
...
- [x] T021 Extend `frontend/tests/accessibility/core.test.tsx` with the ledger in
  resting, open, error, and busy Quick Edit states (axe).
- [x] T022 Add a keyboard walkthrough test: Tab to editable field → Enter opens → pick →
  Enter commits; Escape cancels and returns focus to the trigger (§16).
- [x] T023 Responsive checks: controls wrap correctly at ≤700px, ≥44px targets and
...
- [x] T024 Run full verification: `npm run typecheck -w frontend`, `npm run lint -w
  ...
  backend diffs.
- [x] T025 Walk the Visual Review Checklist (§14); update `frontend/VISUAL_LANGUAGE.md`
